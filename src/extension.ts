...
export function activate(context: vscode.ExtensionContext) {
  ...
  console.log(
    'Congratulations, your extension "createuniqueids" is now active!'
  );
  var random;
  var minimumNumber = 0;
  ...
  function captureMinimumNumber() {
    vscode.window
      .showInputBox({
        placeHolder: "Number",
        prompt: "Enter the minimum number for the range",
        value: "0",
      })
      .then((value) => {
  ...
  function captureMaximumNumber() {
    vscode.window
      .showInputBox({
        placeHolder: "Number",
        prompt: "Enter the maximum number for the range",
        value: "100",
      })
      .then((value) => {
        if (typeof value !== "string") {
...
*/
  ...
  function generate(code: randomIdType) {
    ...
    var value: string =
      code === randomIdType.ShortId ? shortid.generate() : uuid.v4();
    copypaste.copy(value);
  ...
  var disposable = vscode.commands.registerCommand(
    ...
    () => {
      ...
      var items = [
        {
          label: "Generate a Short Id, example = '23TplPdS'",
          description: "",
          code: randomIdType.ShortId,
        },
        {
          label:
            "Generate a UUID/GUID, example = '110ec58a-a0f2-4ac4-8393-c866d813b8d1'",
          description: "",
          code: randomIdType.UUID,
        },
        {
          label: "Generate a random number",
          description: "",
          code: randomIdType.Number,
        },
      ];
